...
async def create_chat_completion(
    messages: str, model_type: str = "gpt-3.5-turbo", functions=None
):
    ...
    if functions is None:
        ...
        response = await openai.ChatCompletion.acreate(
            messages=messages, model=model_type, functions=functions
        )
    return response
...
async def multiple_completions(
    ...
    # this block was recommended in 'openai' code examples
    ...
    tasks = {
        asyncio.ensure_future(
            _func_task(chats[task_id], model_type, functions), loop=nb_loop
        ): task_id
    ...
    # closing client session that we've opened before
